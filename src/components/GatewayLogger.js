          ...
          <div style={{textAlign: 'left', direction: 'ltr'}}>
            <div style={{textAlign: 'center'}}>
              ...
              <CopyToClipboard text={this.state.copied}>
                <Button onClick={() => {
                  let frame = row.uplinkframe ? row.uplinkframe.phypayloadjson : row.downlinkframe.phypayloadjson
                  console.log('copy', frame.macPayload.frmPayload[0].bytes)
                  this.setState({copied: frame.macPayload.frmPayload[0].bytes})
                }} color="info">Copy payload</Button>
              </CopyToClipboard>
            </div>
            <ReactJson
              name={'Frame'}
